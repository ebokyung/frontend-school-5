...
4. 부트스트랩 외주
    ...
    -""""계약서 필히 작성"""" (나중에 문제가 생기는 경우가 꼭 1~2번은 있습니다.)
        - 저는 그런적이 없지만 완성이 안되서 생기는 문제 -> 외주 해보신 분과 함께 하는거 추천
        - 완성 되었으나 기능을 추가해야 해서 생기는 문제 -> '선입금(계약금) 필수'
        - 만족을 못해 입금을 안해서 생기는 문제
        - (1억 이상) 완벽하게 완성 되었으나, 손배로 사업적으로 이득을 챙기려 해서 생기는 문제 -> '카톡 메일 지우지 말 것'
    - 스택
        1. 부트스트랩 + php + mysql
        2. 부트스트랩 + 워드프레스(워드프레스는 DIVI 테마와 Pods 플러그인)
        3. 부트스트랩 + Node, express
        4. 부트스트랩 + Django
5. (자동화순) bootstrap(템플릿 사는 것), tailwindcss(내가 개발), tachyons